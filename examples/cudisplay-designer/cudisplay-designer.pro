...
SOURCES += src/main.cpp \
    src/cudisplay-designer.cpp

HEADERS += \
    src/cudisplay-designer.h

# cuuimake runs uic
# FORMS    = src/quimagetest.ui
...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'quimagetest' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install quimagetest later with `make install` ")
    message("-")
}
