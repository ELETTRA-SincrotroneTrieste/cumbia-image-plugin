...
void QuImgMouseTracker::mouseRelease(QMouseEvent *ev)
{
    ...
    else if(zoomer->enabled() && !mP1.isNull() && ev->button() == Qt::LeftButton) {  // use rect to zoom
        QRect zr = zoomer->zoom(img_d->mapToImg(QRect(mP1, mP2)));
        QList<QRect> zs = zoomer->zoomStack();
        if(img_d->listener) img_d->listener->onZoom(zs.size() > 1 ? zs[zs.size() - 2] : QRect(), zr);
    }
    else if(zoomer->enabled() && !mou_if && ev->button() == Qt::MiddleButton) {
        QList<QRect> zs = zoomer->zoomStack();
